...
@Component(
        property = {
                ...
                "mvc.command.name=/sendPeerEvaluationForm"
        },
        service = MVCActionCommand.class
)
public class SendPeerEvaluationFormMVCActionCommand extends BaseMVCActionCommand {

    Log log = LogFactoryUtil.getLog(SendPeerEvaluationFormMVCActionCommand.class);
    ...
    protected void doProcessAction(ActionRequest actionRequest, ActionResponse actionResponse) throws Exception {
        ...
        for (String user : userStringList) {
            ...
            long appraisalFormTemplateId = sendCurrentYearPeerEvaluationForm(employeeDetails, actionRequest, appraisalEvaluationFormStatus.getAppraisalEvaluationFormStatusId(), appraiseeDetails);
            log.info(generatePeerEvaluationSubmissionLink(actionRequest, appraisalEvaluationFormStatus.getAppraisalEvaluationFormStatusId()));
            ...
            SessionMessages.add(actionRequest, AxHrmsHrAppraisalProcessHrConstants.PEER_FORM_SENT);

        }
    }

    private long sendCurrentYearPeerEvaluationForm(EmployeeDetails employeeDetails, ActionRequest actionRequest, long appraisalFormStatusId, EmployeeDetails appraiseeDetails) throws PortalException {
        TemplateLevelMaster templateLevelMaster = templateLevelMasterLocalService.findBylevelName(AxHrmsHrAppraisalProcessHrConstants.PEER);
        ...
        String body = mailTemplateConfiguration.peerEvaluationFormMailBody();
        String subject = mailTemplateConfiguration.peerEvaluationFormMailSubject();
        ...
        body = body.replace("${PEER_FORM_LINK}", generatePeerEvaluationSubmissionLink(actionRequest, appraisalFormStatusId));
        axHrmsCommonApi.sendMailWithAttachment(
                employeeDetails.getOfficialEmail(), fromEmailAddress,
    ...
    private String generatePeerEvaluationSubmissionLink(ActionRequest actionRequest, long appraisalFormStatusId) {
        //code not prepared
        ThemeDisplay themeDisplay = (ThemeDisplay) actionRequest.getAttribute(WebKeys.THEME_DISPLAY);
        ...
        String[] mvcRenderCommandName = {AxHrmsHrAppraisalProcessHrConstants.SUBMIT_PEER_EVALUATION_FORM_URL};
        ...
        String[] x3 = {String.valueOf(appraisalFormStatusId)};
        programListRenderUrl.getParameterMap().put(AxHrmsHrAppraisalProcessHrConstants.APPRAISAL_EVALUATION_FORM_STATUS_ID, x3);
