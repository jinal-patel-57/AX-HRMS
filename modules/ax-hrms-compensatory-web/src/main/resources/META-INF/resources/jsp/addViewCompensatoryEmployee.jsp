...
<portlet:actionURL name="/addCompensatoryData"
	var="addCompensatoryDataURL" />
<liferay-ui:success key="compensation-request-deleted"
	message="compensation-request-deleted" />
<liferay-ui:success key="compensation-request-submitted"
	message="compensation-request-submitted" />
<link rel="stylesheet"
...
<div class="card">
	<div class="card-header d-flex align-items-center justify-content-end">
		<a class="btn btn-primary btn-sm "
			onclick="open_compensatory_form_modal()"> <liferay-ui:message
				key="request-compensation" />
		</a>
	</div>
	<div class="card-body mb-0">
		<liferay-portlet:renderURL varImpl="iteratorURL" />

		<liferay-ui:search-container total="${projectSearchContainer.total }"
			delta="${delta}" searchContainer="${projectSearchContainer}"
			emptyResultsMessage="no-project-data" iteratorURL="${iteratorURL}">

			<liferay-ui:search-container-results
				results="${projectSearchContainer.results}" />

			<liferay-ui:search-container-row
				className="com.ax.hrms.compensatory.web.dto.CompensatoryDataDto"
				...
				<liferay-ui:search-container-column-text name="Status"
                    ...
                    <%-- <liferay-ui:search-container-column-text
						name="Description" value="${ compensatoryDataDto.getDescription()}"/> --%>
			...
			<liferay-ui:search-iterator
				searchContainer="${projectSearchContainer}" markupView="lexicon"
				paginate="true" />
...
<div class="modal fade" id="addCompensatoryDataModal" tabindex="-1"
	role="dialog" aria-labelledby="addCompensatoryDataModalLabel"
	aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="addCompensatoryDataModalLabel">
					<liferay-ui:message key="add-comp-off" />
				</h5>
			...
			<div class="modal-body">
				<form action="${addCompensatoryDataURL}"
					id="addCompensatoryDataForm" method="post">
					...
					<c:if test="${isManager or isHR}">
						...
						<div class="form-group">
							<input type="hidden" id="<portlet:namespace />hrManagerStatus"
								...
								value="${isManager or isHR}" style="display: none;" /> <label
								for="employeeNamesSelect"> <liferay-ui:message
									key="employee-detail" /><span class="text-danger">*</span> <select
								id="<portlet:namespace />employeeNames"
								name="<portlet:namespace />employeeNames"
								class="custom-select mr-sm-2">
									<option value="">-- Select Employee --</option>
									<c:forEach var="employeeDetailsList"
										items="${employeeDetailsList}">
										<option value="${employeeDetailsList.getEmployeeId()}">${employeeDetailsList.firstName}
											${employeeDetailsList.lastName}
											(${employeeDetailsList.employeeCode})</option>
									</c:forEach>
							</select>
						</div>
					...
					<div class="form-group">
						<label class="" for="compensationDate"><liferay-ui:message
							...
							for="compensationDate"></label>
					</div>
					<div class="form-group">
						<label> <liferay-ui:message key="Duration" /> <span
						...
						<div id="compensationTypeGroup">
							<div class="form-check">
								<input class="form-check-input" type="radio"
									name="<portlet:namespace />compensationType" id="halfDay"
									value="HALF"> <label class="form-check-label"
									for="halfDay"> Half Day </label> <input
									class="ml-2 form-check-input" type="radio"
									name="<portlet:namespace />compensationType" id="fullDay"
									value="FULL"> <label class="form-check-label"
									for="fullDay"> Full Day </label>
							</div>
				...
				<div class="form-group">
					<label for="projectManagerSelect"><liferay-ui:message
						...
						<select id="projectManagerSelect"
							name="<portlet:namespace />projectManager"
							class="custom-select mr-sm-2">
							<!-- adding the user list -->
							<option value=""><liferay-ui:message
									key="select-manager" /></option>
							<c:forEach var="manager" items="${managerList}">
								<option value="${manager.employeeId}">${manager.firstName}
									${manager.lastName}</option>
							</c:forEach>
						</select> <label id="projectManagerSelect-error" class="error text-danger"
...
<div class="modal fade" id="deleteCompensatoryModal" tabindex="-1"
	role="dialog" aria-labelledby="deleteCompensatoryModalLabel"
	aria-hidden="true">
		...
		<div class="modal-content">
			...
			<div class="modal-header  text-white">
				<h5 class="modal-title" id="deleteCompensatoryModalLabel">
					<i class="icon-warning-sign"></i> Confirm Delete
				</h5>
				<button type="button" class="close " data-dismiss="modal">
					<span>&times;</span>
				</button>
			...
			<div class="modal-body text-center">
				...
				<p>Are you sure you want to delete this compensation request?</p>
			</div>
